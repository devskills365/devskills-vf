...
// 🔹 Créer un nouveau plat avec upload d’image
export const createPlat = async (platData) => {
    try {
        ...
        if (platData.image) formData.append('image', platData.image); // ✅ CORRECTION ICI
...
export const updatePlat = async (platId, platData) => {
    try {
        const formData = new FormData();
        for (const key in platData) {
            formData.append(key, platData[key]);
        }

        const response = await API.put(`/admin/plats/${platId}`, formData, {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
        });
